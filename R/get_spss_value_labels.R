get_spss_value_labels <- function(command){
  ...
  # also need to figure out what the delimiter is: " or '
  ...
  tibble(
    ...
    mutate(
      ...
      # labels very well
      col_name = if_else(type == "col_name_value" & str_detect(command, "^[:alpha:]"), str_remove(command, " [:digit:].*$"), NA_character_),
